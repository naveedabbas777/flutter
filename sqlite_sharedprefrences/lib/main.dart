...
class _MyAppState extends State<MyApp> {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: NotesScreen(
          toggleTheme:
              _toggleTheme), // Home screen with theme toggle functionality
    );
  }
}
...
class NotesScreen extends StatefulWidget {
  ...
  const NotesScreen({super.key, required this.toggleTheme});
...
class _NotesScreenState extends State<NotesScreen> {
    ...
    if (_noteController.text.isNotEmpty) {
      if (_editingId == null) {
        await _database.insert(
            'notes', {'content': _noteController.text}); // Insert new note
      } else {
  ...
  Future<void> _deleteNote(int id) async {
    await _database
        .delete('notes', where: 'id = ?', whereArgs: [id]); // Delete note by ID
    _fetchNotes(); // Refresh notes list
  }
                ...
                decoration: InputDecoration(
                  ...
                  suffixIcon: IconButton(
                    ...
                    onPressed: () => _fetchNotes(
                        query: _searchController.text), // Trigger search
                  ),
                ),
              ),
            ),
                    ...
                    trailing: Row(
                      ...
                      children: [
                        ...
                        IconButton(
                          ...
                          onPressed: () =>
                              _deleteNote(_notes[index]['id']), // Delete note
                        ),
                      ],
                    ),
                  );
